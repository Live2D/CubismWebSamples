...
export class LAppLive2DManager {
		...
		for (let i: number = 0; i < this._models.getSize(); i++) {
			if (this._models.at(i).hitTest(LAppDefine.HitAreaNameHead, x, y)) {
				if (LAppDefine.DebugLogEnable) {
					LAppPal.printLog(
						'[APP]hit area: [{0}]',
						LAppDefine.HitAreaNameHead,
					);
				}
				this._models.at(i).setRandomExpression();
			} else if (
				this._models.at(i).hitTest(LAppDefine.HitAreaNameBody, x, y)
			) {
				if (LAppDefine.DebugLogEnable) {
					LAppPal.printLog(
						'[APP]hit area: [{0}]',
	...
	public _viewMatrix: Csm_CubismMatrix44; // モデル描画に用いるview行列
	public _models: Csm_csmVector<LAppModel>; // モデルインスタンスのコンテナ
	public _sceneIndex: number; // 表示するシーンのインデックス値
}
